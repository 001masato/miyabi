# 🀄 麻雀AI「最強雀士」- プロジェクト設計報告書 (Design/Mockup)

**作成日**: 2025-11-07
**ステータス**: ⚠️ **設計・モックアップ段階 (実装コードなし)**
**目的**: 理想的な麻雀AIプロジェクトの設計図とドキュメントサンプルを示すこと

> [!IMPORTANT]
> このドキュメントは、「もし実装されたらこうなる」という**理想状態を記述した設計資料**です。
> 実際のRust/Reactコードはリポジトリに含まれていません。これはフレームワークのドキュメント能力を示すためのサンプルです。
